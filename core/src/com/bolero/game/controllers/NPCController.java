...
public class NPCController extends BaseMapper implements Disposable {
    ...
    public void map(World world) throws FileNotFoundException, MissingPropertyException, NPCDoesNotExistException {
        mapNPCs(world);
        mapSchedules();
    }
        ...
        for (MapObject spawn : spawnObjects) {
            ...
            if (SpawnType.valueOf(type) == SpawnType.npc) {
                ...
                Vector2 spawnPosition = new Vector2(
                        (float) props.get("x") / BoleroGame.UNIT,
                        (float) props.get("y") / BoleroGame.UNIT
                );
    ...
    public void checkSchedules(Clock clock) {
        for (Schedule schedule : schedules) {
            if (clock.getCurrentHour() == schedule.getHour() && clock.getCurrentMinute() == schedule.getMinute()) {
