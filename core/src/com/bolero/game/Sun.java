...
public class Sun {
    private final static float NIGHT = 0.3f;
    private final static float DAY = 1f;
    ...
    public void update(float darkenAmount) {
        ...
        if (timestamp >= BoleroGame.DUSK_END * ratio || timestamp < BoleroGame.DAWN_START * ratio) {
            // NIGHT
            alpha = NIGHT;
        } else if (timestamp >= BoleroGame.DAWN_END * ratio && timestamp < BoleroGame.DUSK_START * ratio) {
            // DAY
            alpha = DAY;
        } else if (timestamp >= BoleroGame.DAWN_START * ratio && timestamp < BoleroGame.DAWN_END * ratio) {
            // DAWN
            // Represented as a linear function (y = mx + c), with x being alpha and y being the timestamp.
